...
(function() {
  ...
  function setConnectionState(label, connected) { 
    ...
    element.classList.toggle('connecting', !connected); 
  }
  ...
  function appendChatMessage(message) { 
    ...
    item.className = `chat-message${message.from === state.clientId ? ' own' : ''}`; 
    ...
    if (message.from !== state.clientId && !$('#chat-panel').classList.contains('open')) { 
      state.unread++; 
      updateChatBadge(); 
    } 
  }
  ...
  function updateMembers(members) { 
    ...
    const connected = members.filter(member => !member.sharing); 
    ...
    $('#members').innerHTML = `
      <section class="member-group transmitting-group">
        <div class="members-title">TRANSMITINDO — <span>${transmitting.length}</span></div>
        ${transmitting.map(renderMember).join('') || '<p class="members-empty">Ninguém transmitindo</p>'}
      </section>
      <section class="member-group connected-group">
        <div class="members-title">NA SALA — <span>${connected.length}</span></div>
        ${connected.map(renderMember).join('')}
      </section>
    `; 
  }

  function getSwitcher() { 
    let element = $('#stream-switcher'); 
    if (!element) { 
      const dock = document.createElement('div'); 
      dock.id = 'stream-dock'; 
      dock.className = 'stream-dock'; 
      const toggle = document.createElement('button'); 
      toggle.id = 'stream-dock-toggle'; 
      toggle.className = 'stream-dock-toggle'; 
      toggle.type = 'button'; 
      toggle.title = 'Ocultar telas'; 
      toggle.setAttribute('aria-label', 'Ocultar telas'); 
      toggle.setAttribute('aria-expanded', 'true'); 
      toggle.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 15 6-6 6 6"/></svg>'; 
      toggle.onclick = () => { 
        const collapsed = dock.classList.toggle('collapsed'); 
        toggle.setAttribute('aria-expanded', String(!collapsed)); 
        toggle.title = collapsed ? 'Mostrar telas' : 'Ocultar telas'; 
        toggle.setAttribute('aria-label', toggle.title); 
      }; 
      element = document.createElement('div'); 
      element.id = 'stream-switcher'; 
      element.className = 'stream-switcher'; 
      dock.append(toggle, element); 
      $('.video-stage').append(dock); 
    } 
    return element; 
  }

  function updateAudioButtons() { 
    getSwitcher().querySelectorAll('.stream-audio-toggle').forEach(button => { 
      const muted = state.mutedStreams.has(button.dataset.source); 
      button.classList.toggle('muted', muted); 
      button.title = muted ? 'Ativar som' : 'Silenciar som'; 
      button.setAttribute('aria-label', button.title); 
      button.innerHTML = muted 
        ? '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m3 3 18 18M9 9v6l4 2V7M17 9.5a5 5 0 0 1 0 5"/></svg>' 
        : '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 10v4h4l5 4V6l-5 4H5ZM18 9a5 5 0 0 1 0 6"/></svg>'; 
    }); 
  }

  function toggleStreamAudio(sourceId, event) { 
    event.stopPropagation(); 
    if (state.mutedStreams.has(sourceId)) {
      state.mutedStreams.delete(sourceId);
    } else {
      state.mutedStreams.add(sourceId);
    }
    if ($('#remote-video')?.srcObject === state.streamSources.get(sourceId)?.stream) {
      $('#remote-video').muted = state.mutedStreams.has(sourceId);
    }
    updateAudioButtons(); 
  }

  function selectStream(sourceId) { 
    const source = state.streamSources.get(sourceId); 
    const video = $('#remote-video'); 
    if (!source || !video) return; 
    video.srcObject = source.stream; 
    video.muted = state.mutedStreams.has(sourceId); 
    video.play().catch(() => {}); 
    const label = $('#video-label'); 
    if (label) label.textContent = `Tela de ${source.name}`; 
    $('#video-stage').classList.add('active'); 
    $('#empty-state').style.display = 'none'; 
    getSwitcher().querySelectorAll('.stream-preview').forEach(button => button.classList.toggle('active', button.dataset.source === sourceId)); 
    updateAudioButtons(); 
  }
  ...
  function renderStreams() { 
    ...
    switcher.innerHTML = ''; 
    state.streamSources.forEach((source, id) => { 
      const card = document.createElement('div'); 
      card.className = 'stream-card'; 
      const previewButton = document.createElement('button'); 
      previewButton.className = 'stream-preview'; 
      previewButton.dataset.source = id; 
      previewButton.type = 'button'; 
      previewButton.title = `Ver a tela de ${source.name}`; 
      previewButton.setAttribute('aria-label', `Selecionar tela de ${source.name}`); 
      const preview = document.createElement('video'); 
      preview.autoplay = true; 
      preview.muted = true; 
      preview.playsInline = true; 
      preview.srcObject = source.stream; 
      const caption = document.createElement('span'); 
      caption.textContent = source.name; 
      previewButton.append(preview, caption); 
      previewButton.onclick = () => selectStream(id); 
      const audioButton = document.createElement('button'); 
      audioButton.className = 'stream-audio-toggle'; 
      audioButton.dataset.source = id; 
      audioButton.type = 'button'; 
      audioButton.disabled = !source.stream.getAudioTracks().length; 
      audioButton.onclick = event => toggleStreamAudio(id, event); 
      card.append(previewButton, audioButton); 
      switcher.append(card); 
    }); 
    if (state.streamSources.size) { 
      const current = [...state.streamSources.keys()].find(id => $('#remote-video')?.srcObject === state.streamSources.get(id).stream); 
      if (!current) selectStream(state.streamSources.keys().next().value); 
    } else { 
      const video = $('#remote-video'); 
      if (video) video.srcObject = null; 
      $('#video-stage').classList.remove('active'); 
      $('#empty-state').style.display = ''; 
    } 
  }

  function addSource(id, stream, name) { 
    if (!stream) return; 
    state.streamSources.set(id, { 
      stream, 
      name: name || state.streamNames.get(id) || state.streamSources.get(id)?.name || `Participante ${id.slice(0, 4)}` 
    }); 
    scheduleRender(); 
  }
  ...
  function addScreenToPeer(peer, stream) { 
    stream.getTracks().forEach(track => { 
      if (track.kind === 'video') track.contentHint = 'detail'; 
      const sender = peer.pc.addTrack(track, stream); 
      if (track.kind === 'video') configureScreenSender(sender); 
    }); 
  }
  ...
  function createPeer(id) {
    ...
    const peer = { 
      ...
      polite: String(state.clientId) > String(id), 
      makingOffer: false, 
      ignoreOffer: false, 
    ...
    peer.pc.onicecandidate = event => { 
      if (event.candidate) send({ type: 'candidate', to: id, candidate: event.candidate }); 
    };
    peer.pc.onnegotiationneeded = async () => { 
      try { 
        peer.makingOffer = true; 
        await peer.pc.setLocalDescription(); 
        send({ type: 'description', to: id, description: peer.pc.localDescription }); 
      } catch (error) { 
    ...
    peer.pc.onconnectionstatechange = () => { 
      if (['failed', 'closed'].includes(peer.pc.connectionState)) removePeer(id); 
    };
    if (state.stream) { 
      addScreenToPeer(peer, state.stream); 
  ...
  async function handleDescription(message) { 
    ...
    try { 
      ...
      if (description.type === 'offer') { 
        await peer.pc.setLocalDescription(); 
        send({ type: 'description', to: message.from, description: peer.pc.localDescription }); 
      } 
    } catch (error) { 
  ...
  async function handleMessage(message) {
    if (message.type === 'welcome') { 
      ...
      state.isOwner = message.members.find(member => member.id === state.clientId)?.owner || false; 
      updateMembers(message.members); 
      setConnectionState('Conectado', true); 
      $('#loading-state').style.display = 'none'; 
      toast('Conectado à sala.'); 
      message.members.filter(member => member.id !== state.clientId).forEach(member => { 
        createPeer(member.id); 
        if (member.sharing) state.streamNames.set(member.id, member.name); 
      }); 
    } else if (message.type === 'user-joined') { 
      updateMembers(message.members); 
      createPeer(message.id); 
      ...
      state.socket?.close(); 
      toast(`Você foi expulso da sala por ${message.name}.`); 
      ...
      state.streamNames.set(message.from, message.name); 
      const source = state.streamSources.get(message.from); 
      if (source) source.name = message.name; 
      scheduleRender(); 
    } else if (message.type === 'stop-sharing') { 
      state.streamNames.delete(message.from); 
      state.streamSources.delete(message.from); 
      scheduleRender(); 
    }
  }
  ...
  function startSharing() { 
    return navigator.mediaDevices.getDisplayMedia({ 
      ...
      addSource(state.clientId, stream, nickname); 
      $('#control-stop').disabled = false; 
      send({ type: 'sharing-started', name: nickname }); 
      toast('Compartilhamento iniciado em alta qualidade.'); 
      state.peers.forEach(peer => { 
        if (!peer.streamAdded) { 
          addScreenToPeer(peer, stream); 
          peer.streamAdded = true; 
        } 
      }); 
    }).catch(error => { 
      if (error.name !== 'NotAllowedError') toast('Não foi possível iniciar o compartilhamento.'); 
    }); 
  }
  ...
  function copy(value, message) { 
    navigator.clipboard?.writeText(value).then(() => toast(message)).catch(() => toast('Não foi possível copiar.')); 
  }
  ...
  function requestNicknameForJoin() {
    ...
    form.addEventListener('submit', event => {
      ...
      if (!value) { 
        gate.querySelector('#room-nickname-error').textContent = 'Digite um nome para continuar.'; 
        return; 
      }
  ...
  function initializeRoom() {
    ...
    $('#members').innerHTML = `<div class="member">
      <div class="avatar member-avatar">${initials(nickname)}</div>
      <div class="member-info">
        <strong>${escapeHtml(nickname)} <span class="you-badge">VOCÊ</span></strong>
        <small>Você está na sala</small>
      </div>
      <span class="member-share-icon" title="Não está transmitindo tela" aria-label="Não está transmitindo tela">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <rect x="3" y="4" width="18" height="13" rx="2"/>
          <path d="M8 21h8M12 17v4"/>
          <path d="m5 7 14 14" opacity=".7"/>
        </svg>
      </span>
      <i class="member-status online"></i>
    </div>`;
    ...
    try {
      ...
      state.socket.onopen = () => {
        ...
        console.log('📤 Enviando join com room:', roomId, 'e nome:', nickname);
        if (notFound) notFound.style.display = 'none';
        if (loadingState) loadingState.style.display = 'none';
