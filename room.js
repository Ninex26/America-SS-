...
// Aceita IDs com 4-8 caracteres, opcionalmente com hífen
const roomPattern = /^[A-Z0-9]{4,8}(-[A-Z0-9]{4,8})?$/;
...
document.title = 'DuArrasta | Sala';
document.querySelectorAll('.brand strong').forEach(element => { element.textContent = 'DuArrasta'; });
document.querySelectorAll('.brand').forEach(element => element.setAttribute('aria-label', 'DuArrasta início'));
document.querySelectorAll('.logo-mark img').forEach(element => { element.src = logoUrl; element.alt = 'Logo DuArrasta'; });
const state = { socket: null, clientId: null, isOwner: false, peers: new Map(), stream: null, streamSources: new Map(), streamNames: new Map(), mutedStreams: new Set(), unread: 0 };
...
function escapeHtml(value) {
  const map = { '&': String.fromCharCode(38) + 'amp;', '<': String.fromCharCode(38) + 'lt;', '>': String.fromCharCode(38) + 'gt;', '"': String.fromCharCode(38) + 'quot;', "'": String.fromCharCode(38) + '#39;' };
...
function toggleChat(open) { const panel = $('#chat-panel'); const button = $('#chat-toggle'); panel.classList.toggle('open', open); panel.setAttribute('aria-hidden', String(!open)); button.setAttribute('aria-expanded', String(open)); if (open) { state.unread = 0; updateChatBadge(); $('#chat-input').focus(); } }
function updateChatBadge() { const badge = $('#chat-badge'); if (!badge) return; badge.hidden = state.unread === 0; badge.textContent = state.unread > 99 ? '99+' : state.unread; }
function appendChatMessage(message) { const empty = $('#chat-empty'); if (empty) empty.remove(); const item = document.createElement('article'); item.className = `chat-message${message.from === state.clientId ? ' own' : ''}`; const header = document.createElement('div'); header.className = 'chat-message-header'; const name = document.createElement('strong'); name.textContent = message.name; const time = document.createElement('time'); time.textContent = new Date(message.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); header.append(name, time); const body = document.createElement('p'); body.textContent = message.text; item.append(header, body); $('#chat-messages').append(item); $('#chat-messages').scrollTop = $('#chat-messages').scrollHeight; if (message.from !== state.clientId && !$('#chat-panel').classList.contains('open')) { state.unread++; updateChatBadge(); } }
...
function updateMembers(members) { const transmitting = members.filter(member => member.sharing); const connected = members.filter(member => !member.sharing); $('#user-count').textContent = members.length; $('#member-count').textContent = members.length; const renderMember = member => {
  const shareIcon = member.sharing
  ...
  return `<div class="member ${member.sharing ? 'is-sharing' : ''}"><div class="avatar member-avatar">${initials(member.name)}</div><div class="member-info"><strong>${escapeHtml(member.name)} ${member.owner ? '<span class="owner-crown" title="Dono da sala" aria-label="Dono da sala"><svg viewBox="0 0 24 24"><path d="m3 7 4 4 5-7 5 7 4-4-2 12H5L3 7Z"/><path d="M5 19h14"/></svg></span>' : ''} ${member.id === state.clientId ? '<span class="you-badge">VOCÊ</span>' : ''}</strong><small>${member.sharing ? 'Compartilhando tela' : member.id === state.clientId ? 'Você está na sala' : 'Conectado'}</small></div>${state.isOwner && member.id !== state.clientId ? `<button class="kick-button" type="button" title="Expulsar ${escapeHtml(member.name)}" aria-label="Expulsar ${escapeHtml(member.name)}" onclick="kickMember('${member.id}', '${escapeHtml(member.name).replace(/'/g, "\\'")}')"><svg viewBox="0 0 24 24"><path d="M10 17l5-5-5-5M15 12H3M21 4v16"/></svg></button>` : ''}${shareIcon}<i class="member-status ${member.sharing ? '' : 'online'}"></i></div>`;
}; $('#members').innerHTML = `<section class="member-group transmitting-group"><div class="members-title">TRANSMITINDO — <span>${transmitting.length}</span></div>${transmitting.map(renderMember).join('') || '<p class="members-empty">Ninguém transmitindo</p>'}</section><section class="member-group connected-group"><div class="members-title">NA SALA — <span>${connected.length}</span></div>${connected.map(renderMember).join('')}</section>`; }
...
function createPeer(id) {
  ...
  const peer = { pc: new RTCPeerConnection({ iceServers: [{ urls: 'stun:stun.l.google.com:19302' }, { urls: 'stun:stun1.l.google.com:19302' }, { urls: 'stun:stun.cloudflare.com:3478' }] }), polite: String(state.clientId) > String(id), makingOffer: false, ignoreOffer: false, remoteDescriptionSet: false, pendingCandidates: [], remoteStream: new MediaStream(), streamAdded: false };
  ...
  peer.pc.ontrack = event => {
    ...
    if (!stream._trackHandlerSet) {
      stream._trackHandlerSet = true;
      stream.onaddtrack = () => addSource(id, stream, state.streamSources.get(id)?.name);
      stream.onremovetrack = () => { if (!stream.getTracks().length) { state.streamSources.delete(id); scheduleRender(); } };
    }
    addSource(id, stream, state.streamSources.get(id)?.name);
    const video = $('#remote-video');
    if (video.srcObject === stream) video.play().catch(() => {});
  };
  peer.pc.onconnectionstatechange = () => { if (['failed', 'closed'].includes(peer.pc.connectionState)) removePeer(id); };
  if (state.stream) { addScreenToPeer(peer, state.stream); peer.streamAdded = true; }
  return peer;
}
function removePeer(id) { state.peers.get(id)?.pc.close(); state.peers.delete(id); state.streamSources.delete(id); scheduleRender(); }
...
async function handleMessage(message) {
  if (message.type === 'welcome') { state.clientId = message.id; state.isOwner = message.members.find(member => member.id === state.clientId)?.owner || false; updateMembers(message.members); setConnectionState('Conectado', true); $('#loading-state').style.display = 'none'; toast('Conectado à sala.'); message.members.filter(member => member.id !== state.clientId).forEach(member => { createPeer(member.id); if (member.sharing) state.streamNames.set(member.id, member.name); }); }
  ...
  else if (message.type === 'sharing-started') { state.streamNames.set(message.from, message.name); const source = state.streamSources.get(message.from); if (source) source.name = message.name; scheduleRender(); }
  else if (message.type === 'stop-sharing') { state.streamNames.delete(message.from); state.streamSources.delete(message.from); scheduleRender(); }
}
function startSharing() { return navigator.mediaDevices.getDisplayMedia({ video: { cursor: 'always', frameRate: { ideal: 30, max: 30 }, width: { ideal: 2560, max: 3840 }, height: { ideal: 1440, max: 2160 } }, audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true } }).then(stream => { state.stream = stream; const track = stream.getVideoTracks()[0]; track.contentHint = 'detail'; track.onended = stopSharing; addSource(state.clientId, stream, nickname); $('#control-stop').disabled = false; send({ type: 'sharing-started', name: nickname }); toast('Compartilhamento iniciado em alta qualidade.'); state.peers.forEach(peer => { if (!peer.streamAdded) { addScreenToPeer(peer, stream); peer.streamAdded = true; } }); }).catch(error => { if (error.name !== 'NotAllowedError') toast('Não foi possível iniciar o compartilhamento.'); }); }
function stopSharing() { if (!state.stream) return; state.stream.getTracks().forEach(track => track.stop()); state.stream = null; state.streamSources.delete(state.clientId); state.peers.forEach(peer => { peer.pc.getSenders().forEach(sender => { if (sender.track && sender.track.kind === 'video') peer.pc.removeTrack(sender); }); peer.streamAdded = false; }); scheduleRender(); $('#control-stop').disabled = true; send({ type: 'stop-sharing' }); }
...
// FORÇA OCULTAÇÃO DA MENSAGEM DE ERRO
const notFound = document.getElementById('not-found');
const emptyState = document.getElementById('empty-state');
const loadingState = document.getElementById('loading-state');

// Esconde apenas o erro (o estado vazio deve ficar visível até alguém transmitir)
if (notFound) notFound.style.display = 'none';

renderStreams();
$('#room-code-label').textContent = roomId || '????'; 
$('#sidebar-code').textContent = roomId || '????'; 
$('#footer-name').textContent = nickname; 
const footerAvatar = $('#footer-avatar');
if (footerAvatar) footerAvatar.textContent = initials(nickname);
$('#members').innerHTML = `<div class="member"><div class="avatar member-avatar">${initials(nickname)}</div><div class="member-info"><strong>${escapeHtml(nickname)} <span class="you-badge">VOCÊ</span></strong><small>Você está na sala</small></div><span class="member-share-icon" title="Não está transmitindo tela" aria-label="Não está transmitindo tela"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="13" rx="2"/><path d="M8 21h8M12 17v4"/><path d="m5 7 14 14" opacity=".7"/></svg></span><i class="member-status online"></i></div>`;

// Se não tiver ID, mostra erro
if (!roomId || roomId.trim() === '') { 
  console.error('Room ID está vazio!');
...
$('#share-screen').onclick = startSharing; 
$('#control-share').onclick = startSharing; 
$('#control-stop').onclick = stopSharing; 
$('#copy-link').onclick = () => copy(location.href, 'Link copiado!'); 
$('#empty-copy-link').onclick = () => copy(location.href, 'Link copiado!'); 
$('#control-copy').onclick = () => copy(location.href, 'Link copiado!'); 
$('#copy-code').onclick = () => copy(roomId, 'Código copiado!'); 
$('#control-fullscreen').onclick = () => document.documentElement.requestFullscreen?.(); 
...
$('#chat-toggle').onclick = () => toggleChat(!$('#chat-panel').classList.contains('open')); 
